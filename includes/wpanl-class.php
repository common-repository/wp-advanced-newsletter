...
if( ! class_exists( 'wpanl' ) ) 
{
	...
	class wpanl {
		...
		function ced_mailchimp_send_mail()
		{
			...
			//echo WANL_DIR.'email-service/mailchimp-api/src/Mailchimp/MAPI.class.php';
		...
		function prefix_ajax_activecamp_send_mail()
		{
			...
			if(isset($camp_details) && $camp_details->result_code == 1)
			{
				...
				$api_u = "contact/list_?ids='.$campaign_list_id.'";
				$api_u = "contact/list_?ids=all";
				$camp_users = $ac->api($api_u);
				foreach ($camp_users as $val){
					echo "<pre>"; print_r($val->email); die;
					//$camp_details = $ac->api("campaign/send?campaignid=$campaign_id&messageid=$campaign_msg_id&type=mime&action=send&email=$val->email");
